...
func (h *Handler) handleTableInfo(w http.ResponseWriter, r *http.Request) {
    ...
    case "sqlite":
        ...
        type row struct{ name, ctype string; notnull int; dflt_value any }
        var rs []row
...
func (h *Handler) handleBrowseRows(w http.ResponseWriter, r *http.Request) {
    ...
    switch driver {
    case "postgres":
        if schema == "" { sqlStr = "SELECT * FROM \"" + table + "\" LIMIT ? OFFSET ?"; args = []any{limit, offset} } else { sqlStr = "SELECT * FROM \"" + schema + "\".\"" + table + "\" LIMIT ? OFFSET ?"; args = []any{limit, offset} }
    case "mysql":
        if schema == "" { WriteError(w, r, "schema required for mysql"); return }
        sqlStr = "SELECT * FROM `" + schema + "`.`" + table + "` LIMIT ? OFFSET ?"; args = []any{limit, offset}
    case "sqlite":
        sqlStr = "SELECT * FROM `" + table + "` LIMIT ? OFFSET ?"; args = []any{limit, offset}
    case "sqlserver":
        // Without ORDER BY, OFFSET is not allowed; use TOP and ignore offset for now.
        if schema == "" { sqlStr = "SELECT TOP (?) * FROM [" + table + "]"; args = []any{limit} } else { sqlStr = "SELECT TOP (?) * FROM [" + schema + "].[" + table + "]"; args = []any{limit} }
    default:
        WriteError(w, r, "unsupported driver")
        return
    }
...
func (h *Handler) handleListTables(w http.ResponseWriter, r *http.Request) {
    ...
    case "postgres":
        ...
        err = s.Conn.DB.Raw("SELECT table_name AS name FROM information_schema.tables WHERE table_schema = ? ORDER BY name", schema).Scan(&rows).Error
    case "mysql":
        ...
        err = s.Conn.DB.Raw("SELECT table_name AS name FROM information_schema.tables WHERE table_schema = ? ORDER BY name", schema).Scan(&rows).Error
    case "sqlite":
        err = s.Conn.DB.Raw("SELECT name AS name FROM sqlite_master WHERE type IN ('table','view') ORDER BY name").Scan(&rows).Error
    case "sqlserver":
        if schema == "" {
            schema = "dbo"
        }
        err = s.Conn.DB.Raw("SELECT t.name AS name FROM sys.tables t JOIN sys.schemas s ON t.schema_id=s.schema_id WHERE s.name = ? ORDER BY name", schema).Scan(&rows).Error
    default:
        WriteError(w, r, "unsupported driver")
        return
    }
    ...
    WriteSuccessWithData(w, r, "ok", map[string]any{"tables": names})
}
